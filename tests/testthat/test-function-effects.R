...
test_that('picrust results gives similar results for ml and hmc',{
  ...
  suppressWarnings({
    ml <- est(GEVERS$functions,level=2,iters=5,method='ml')
    hmc <- est(GEVERS$functions,level=2,iters=25,chains=1,
               return_summary=FALSE,
               prior=c('t','t','normal'),
               t_df=c(7,7),seed=123)
    hmc_summary <- extract(hmc)
    resume_orig <- resume(hmc,init_type='orig',iters=50,chains=1,return_summary=FALSE,seed=123)
...
test_that('tax4fun results gives similar results for ml and hmc',{
  ...
  suppressWarnings({
    ml <- est(DAVID$functions,level=2,iters=5,method='ml')
    hmc <- est(DAVID$functions,level=2,iters=25,chains=1,
               return_summary=FALSE,
               prior=c('t','t','normal'),
               t_df=c(7,7),seed=123)
    hmc_summary <- extract(hmc)
